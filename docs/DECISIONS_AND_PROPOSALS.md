...
| 우선순위 | 제안 | 판단 기준 |
| ---: | --- | --- |
| 1 | region `16`에서 성공 반경 `18mm`를 먼저 확인 | `20mm` 성공과 `15mm` 실패 사이의 실제 난도 차이를 분리 |
| 2 | `18mm`가 통과하면 final 25mm 이내 center/top-down XY reward를 보강 | `15mm` gate 직행 실패가 reward slope 부족인지 확인 |
| 3 | 보강 조건으로 region `16`의 `15mm`를 재시도 | 성공 `10`회 또는 명확한 실패 로그 확보 |
| 4 | region `16` 통과 조건을 region `17`에 복제 | 두 focus region 모두 같은 조건에서 재현되는지 확인 |
| 5 | `16/17`이 `15mm`를 통과한 뒤 `16..20` 확장 | 전체 `15..25` 재시도 전에 가까운 실패 대역부터 검증 |
